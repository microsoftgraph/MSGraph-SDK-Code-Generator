...
Get-ChildItem -Path $env:MainDirectory -Exclude "microsoft_graph_request_adapter.rb", "microsoft_graph_service_client.rb", "version_information.rb" | Where-Object { !$_.PSIsContainer -and $_.Name -like "*.rb" } | ForEach-Object { Remove-Item -Path $_.FullName -Verbose -Force}
