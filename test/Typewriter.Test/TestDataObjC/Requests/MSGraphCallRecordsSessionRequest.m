...
@implementation MSGraphSessionRequest
...
- (MSURLSessionDataTask *)getWithCompletion:(void (^)(MSGraphSession *response, NSError *error))completionHandler
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self get]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphSession alloc] initWithDictionary:response];
                                        }
...
- (NSMutableURLRequest *)update:(MSGraphSession *)session
{
    NSData *body = [NSJSONSerialization dataWithJSONObject:[session dictionaryFromItem] options:0 error:nil];
...
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self update:session]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphSession alloc] initWithDictionary:response];
                                        }
                                              completion:completionHandler];
