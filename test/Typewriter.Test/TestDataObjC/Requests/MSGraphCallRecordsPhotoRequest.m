...
@implementation MSGraphPhotoRequest
...
- (MSURLSessionDataTask *)getWithCompletion:(void (^)(MSGraphPhoto *response, NSError *error))completionHandler
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self get]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphPhoto alloc] initWithDictionary:response];
                                        }
...
- (NSMutableURLRequest *)update:(MSGraphPhoto *)photo
{
    NSData *body = [NSJSONSerialization dataWithJSONObject:[photo dictionaryFromItem] options:0 error:nil];
...
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self update:photo]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphPhoto alloc] initWithDictionary:response];
                                        }
                                              completion:completionHandler];
