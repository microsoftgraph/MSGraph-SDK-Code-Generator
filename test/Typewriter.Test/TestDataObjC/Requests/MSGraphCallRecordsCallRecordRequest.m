...
@implementation MSGraphCallRecordRequest
...
- (MSURLSessionDataTask *)getWithCompletion:(void (^)(MSGraphCallRecord *response, NSError *error))completionHandler
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self get]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphCallRecord alloc] initWithDictionary:response];
                                        }
...
- (NSMutableURLRequest *)update:(MSGraphCallRecord *)callRecord
{
    NSData *body = [NSJSONSerialization dataWithJSONObject:[callRecord dictionaryFromItem] options:0 error:nil];
...
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self update:callRecord]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphCallRecord alloc] initWithDictionary:response];
                                        }
                                              completion:completionHandler];
