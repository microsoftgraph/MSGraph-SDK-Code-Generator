...
@implementation MSGraphSingletonEntity2Request
...
- (MSURLSessionDataTask *)getWithCompletion:(void (^)(MSGraphSingletonEntity2 *response, NSError *error))completionHandler
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self get]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphSingletonEntity2 alloc] initWithDictionary:response];
                                        }
...
- (NSMutableURLRequest *)update:(MSGraphSingletonEntity2 *)singletonEntity2
{
    NSData *body = [NSJSONSerialization dataWithJSONObject:[singletonEntity2 dictionaryFromItem] options:0 error:nil];
...
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self update:singletonEntity2]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphSingletonEntity2 alloc] initWithDictionary:response];
                                        }
                                              completion:completionHandler];
