...
@implementation MSGraphCloudCommunicationsRequest
...
- (MSURLSessionDataTask *)getWithCompletion:(void (^)(MSGraphCloudCommunications *response, NSError *error))completionHandler
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self get]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphCloudCommunications alloc] initWithDictionary:response];
                                        }
...
- (NSMutableURLRequest *)update:(MSGraphCloudCommunications *)cloudCommunications
{
    NSData *body = [NSJSONSerialization dataWithJSONObject:[cloudCommunications dictionaryFromItem] options:0 error:nil];
...
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self update:cloudCommunications]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphCloudCommunications alloc] initWithDictionary:response];
                                        }
                                              completion:completionHandler];
