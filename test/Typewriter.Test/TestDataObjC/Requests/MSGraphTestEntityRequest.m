...
@implementation MSGraphTestEntityRequest
...
- (MSURLSessionDataTask *)getWithCompletion:(void (^)(MSGraphTestEntity *response, NSError *error))completionHandler
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self get]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphTestEntity alloc] initWithDictionary:response];
                                        }
...
- (NSMutableURLRequest *)update:(MSGraphTestEntity *)testEntity
{
    NSData *body = [NSJSONSerialization dataWithJSONObject:[testEntity dictionaryFromItem] options:0 error:nil];
...
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self update:testEntity]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphTestEntity alloc] initWithDictionary:response];
                                        }
                                              completion:completionHandler];
