...
@implementation MSGraphOptionRequest
...
- (MSURLSessionDataTask *)getWithCompletion:(void (^)(MSGraphOption *response, NSError *error))completionHandler
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self get]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphOption alloc] initWithDictionary:response];
                                        }
...
- (NSMutableURLRequest *)update:(MSGraphOption *)option
{
    NSData *body = [NSJSONSerialization dataWithJSONObject:[option dictionaryFromItem] options:0 error:nil];
...
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self update:option]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphOption alloc] initWithDictionary:response];
                                        }
                                              completion:completionHandler];
