...
@implementation MSGraphEntityType3Request
...
- (MSURLSessionDataTask *)getWithCompletion:(void (^)(MSGraphEntityType3 *response, NSError *error))completionHandler
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self get]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphEntityType3 alloc] initWithDictionary:response];
                                        }
...
- (NSMutableURLRequest *)update:(MSGraphEntityType3 *)entityType3
{
    NSData *body = [NSJSONSerialization dataWithJSONObject:[entityType3 dictionaryFromItem] options:0 error:nil];
...
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self update:entityType3]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphEntityType3 alloc] initWithDictionary:response];
                                        }
                                              completion:completionHandler];
