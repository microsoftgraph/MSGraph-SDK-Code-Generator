...
@implementation MSGraphScheduleRequest
...
- (MSURLSessionDataTask *)getWithCompletion:(void (^)(MSGraphSchedule *response, NSError *error))completionHandler
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self get]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphSchedule alloc] initWithDictionary:response];
                                        }
...
- (NSMutableURLRequest *)update:(MSGraphSchedule *)schedule
{
    NSData *body = [NSJSONSerialization dataWithJSONObject:[schedule dictionaryFromItem] options:0 error:nil];
...
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self update:schedule]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphSchedule alloc] initWithDictionary:response];
                                        }
                                              completion:completionHandler];
