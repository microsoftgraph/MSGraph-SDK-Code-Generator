...
@implementation MSGraphEndpointRequest
...
- (MSURLSessionDataTask *)getWithCompletion:(void (^)(MSGraphEndpoint *response, NSError *error))completionHandler
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self get]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphEndpoint alloc] initWithDictionary:response];
                                        }
...
- (NSMutableURLRequest *)update:(MSGraphEndpoint *)endpoint
{
    NSData *body = [NSJSONSerialization dataWithJSONObject:[endpoint dictionaryFromItem] options:0 error:nil];
...
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self update:endpoint]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphEndpoint alloc] initWithDictionary:response];
                                        }
                                              completion:completionHandler];
