...
- (MSURLSessionDataTask *)getWithCompletion:(void (^)(MSGraphTimeOff *response, NSError *error))completionHandler
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self get]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphTimeOff alloc] initWithDictionary:response];
                                        }
...
- (NSMutableURLRequest *)update:(MSGraphTimeOff *)timeOff
{
    NSData *body = [NSJSONSerialization dataWithJSONObject:[timeOff dictionaryFromItem] options:0 error:nil];
...
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self update:timeOff]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphTimeOff alloc] initWithDictionary:response];
                                        }
                                              completion:completionHandler];
