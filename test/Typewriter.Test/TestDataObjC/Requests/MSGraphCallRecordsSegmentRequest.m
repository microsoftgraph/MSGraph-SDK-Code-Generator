...
@implementation MSGraphSegmentRequest
...
- (MSURLSessionDataTask *)getWithCompletion:(void (^)(MSGraphSegment *response, NSError *error))completionHandler
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self get]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphSegment alloc] initWithDictionary:response];
                                        }
...
- (NSMutableURLRequest *)update:(MSGraphSegment *)segment
{
    NSData *body = [NSJSONSerialization dataWithJSONObject:[segment dictionaryFromItem] options:0 error:nil];
...
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self update:segment]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphSegment alloc] initWithDictionary:response];
                                        }
                                              completion:completionHandler];
