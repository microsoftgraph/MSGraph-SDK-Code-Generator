...
@implementation MSGraphTestTypeRequest
...
- (MSURLSessionDataTask *)getWithCompletion:(void (^)(MSGraphTestType *response, NSError *error))completionHandler
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self get]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphTestType alloc] initWithDictionary:response];
                                        }
...
- (NSMutableURLRequest *)update:(MSGraphTestType *)testType
{
    NSData *body = [NSJSONSerialization dataWithJSONObject:[testType dictionaryFromItem] options:0 error:nil];
...
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self update:testType]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphTestType alloc] initWithDictionary:response];
                                        }
                                              completion:completionHandler];
