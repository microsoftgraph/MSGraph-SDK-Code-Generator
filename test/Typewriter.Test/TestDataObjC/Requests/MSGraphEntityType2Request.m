...
@implementation MSGraphEntityType2Request
...
- (MSURLSessionDataTask *)getWithCompletion:(void (^)(MSGraphEntityType2 *response, NSError *error))completionHandler
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self get]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphEntityType2 alloc] initWithDictionary:response];
                                        }
...
- (NSMutableURLRequest *)update:(MSGraphEntityType2 *)entityType2
{
    NSData *body = [NSJSONSerialization dataWithJSONObject:[entityType2 dictionaryFromItem] options:0 error:nil];
...
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self update:entityType2]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphEntityType2 alloc] initWithDictionary:response];
                                        }
                                              completion:completionHandler];
